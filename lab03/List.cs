...
namespace lab03
{
    ...
    public class List
    {
        ...
        {
            private readonly int _organisationID;//можно установить только один раз
            public string OrganisationName { get; set; }
            //конструкторы
            public Production()
            {
                _organisationID = GetHashCode();//устанавливается в результате вызова метода
            }
        ...
        {
            if (_count == 0)
                throw new InvalidOperationException();//исключение чтобы сигнализировать о недопустимой операции.

            Node current = Head;// Хранит ссылку на текущий узел
            Node previous = null;//в начале нет предыдущего узла

            while (current != null)
            {
                if (current.Data == data)//соответствуют ли данные текущего узла
                {
                    _count--;//удаление узла из списка
                    if (previous == null)
                    {
                        Head = Head.Next;
                        if (Head == null)//список стал пустым
                        {
                            Tail = null;
                        }
                        return;
